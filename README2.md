...
| ----------------- | ------ | ---------------------------------------------- |
| `x = 10;`         | ✅     | IDENT(x), ASSIGN, INT(10), SEMICOLON           |
| `float_val = 3.14;` | ✅   | IDENT, ASSIGN, FLOAT(3.14), SEMICOLON          |
| `money = 100$;`   | ❌     | Lexer Error: Illegal character `$`             |
| `if(x)`           | ✅     | IF, LPAREN, IDENT, RPAREN                      |
...
| --------------------------- | ------ | ------------------------ |
| `if (x > 5) { print(x); }` | ✅     | Generates `If` node      |
| `x = 5 + 3 * 2;`           | ✅     | Correct precedence       |
...
| Input                      | Status | Result         |
| -------------------------- | ------ | -------------- |
| `x = 5; print(x);`        | ✅     | Valid IR       |
| `def f(){ y=1; } print(y);` | ❌   | Scope error    |
| `while(x<5){...}`         | ✅     | Loop IR        |
